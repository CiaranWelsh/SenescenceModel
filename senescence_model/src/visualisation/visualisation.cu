...
//	createVBO(
// &sphereVerts,
// SPHERE_SLICES* (SPHERE_STACKS+1) * sizeof(glm::vec3));
...
//	void createTBO(cudaGraphicsResource_t* cudaResource,
// GLuint* tbo,
// GLuint* tex,
// GLuint size)
//	{
...
//		checkGLError();
//	}
		// texture buffers for TissueBlock stuff
		glGenBuffers( 1, &TissueBlock_default_tbo);
		glBindBuffer( GL_TEXTURE_BUFFER_EXT, TissueBlock_default_tbo);

		// initialize buffer object
		glBufferData(
				GL_TEXTURE_BUFFER_EXT,
				xmachine_memory_TissueBlock_MAX * sizeof( glm::vec4),
				0,
				GL_DYNAMIC_DRAW);

		//tex
		glGenTextures(1, &TissueBlock_default_displacementTex);
		glBindTexture(GL_TEXTURE_BUFFER_EXT, TissueBlock_default_displacementTex);
		glTexBufferEXT(GL_TEXTURE_BUFFER_EXT, GL_RGBA32F_ARB, TissueBlock_default_tbo);
		glBindBuffer(GL_TEXTURE_BUFFER_EXT, 0);

		// register buffer object with CUDA
		gpuErrchk(cudaGraphicsGLRegisterBuffer(
				&TissueBlock_default_cgr,
				TissueBlock_default_tbo,
...
//			&TissueBlock_default_cgr,
//			&TissueBlock_default_tbo,
//			&TissueBlock_default_displacementTex,
//			xmachine_memory_TissueBlock_MAX * sizeof( glm::vec4));
