...
/* Non partitioned, spatial partitioned and on-graph partitioned message variables  */
__constant__ int d_message_location_report_count;         /**< message list counter*/
__constant__ int d_message_location_report_output_type;   /**< message output type (single or optional)*/
//Spatial Partitioning Variables
__constant__ glm::vec3 d_message_location_report_min_bounds;           /**< min bounds (x,y,z) of partitioning environment */
__constant__ glm::vec3 d_message_location_report_max_bounds;           /**< max bounds (x,y,z) of partitioning environment */
__constant__ glm::ivec3 d_message_location_report_partitionDim;           /**< partition dimensions (x,y,z) of partitioning environment */
__constant__ float d_message_location_report_radius;                 /**< partition radius (used to determin the size of the partitions) */
...
/* location_report Message Bindings */texture<int, 1, cudaReadModeElementType> tex_xmachine_message_location_report_id;
__constant__ int d_tex_xmachine_message_location_report_id_offset;texture<float, 1, cudaReadModeElementType> tex_xmachine_message_location_report_x;
__constant__ int d_tex_xmachine_message_location_report_x_offset;texture<float, 1, cudaReadModeElementType> tex_xmachine_message_location_report_y;
__constant__ int d_tex_xmachine_message_location_report_y_offset;texture<float, 1, cudaReadModeElementType> tex_xmachine_message_location_report_z;
__constant__ int d_tex_xmachine_message_location_report_z_offset;
texture<int, 1, cudaReadModeElementType> tex_xmachine_message_location_report_pbm_start;
__constant__ int d_tex_xmachine_message_location_report_pbm_start_offset;
texture<int, 1, cudaReadModeElementType> tex_xmachine_message_location_report_pbm_end_or_count;
__constant__ int d_tex_xmachine_message_location_report_pbm_end_or_count_offset;
 ...
 * @param damage agent variable of type float
 */
template <int AGENT_TYPE>
__device__ void add_Fibroblast_agent(xmachine_memory_Fibroblast_list* agents, int id, float x, float y, float z, float doublings, float damage){
...
//non templated version assumes DISCRETE_2D but works also for CONTINUOUS
__device__ void add_Fibroblast_agent(xmachine_memory_Fibroblast_list* agents, int id, float x, float y, float z, float doublings, float damage){
    add_Fibroblast_agent<DISCRETE_2D>(agents, id, x, y, z, doublings, damage);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created location_report message functions */


/** add_location_report_message
 * Add non partitioned or spatially partitioned location_report message
 * @param messages xmachine_message_location_report_list message list to add too
 * @param id agent variable of type int
 * @param x agent variable of type float
 * @param y agent variable of type float
 * @param z agent variable of type float
 */
__device__ void add_location_report_message(xmachine_message_location_report_list* messages, int id, float x, float y, float z){

	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x + d_message_location_report_count;

	int _position;
	int _scan_input;

	//decide output position
	if(d_message_location_report_output_type == single_message){
		_position = index; //same as agent position
		_scan_input = 0;
	}else if (d_message_location_report_output_type == optional_message){
		_position = 0;	   //to be calculated using Prefix sum
		_scan_input = 1;
	}

	//AoS - xmachine_message_location_report Coalesced memory write
	messages->_scan_input[index] = _scan_input;	
	messages->_position[index] = _position;
	messages->id[index] = id;
	messages->x[index] = x;
...
/**
 * Scatter non partitioned or spatially partitioned location_report message (for optional messages)
 * @param messages scatter_optional_location_report_messages Sparse xmachine_message_location_report_list message list
 * @param message_swap temp xmachine_message_location_report_list message list to scatter sparse messages to
 */
__global__ void scatter_optional_location_report_messages(xmachine_message_location_report_list* messages, xmachine_message_location_report_list* messages_swap){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
	...
	//if optional message is to be written
	if (_scan_input == 1){
		int output_index = messages_swap->_position[index] + d_message_location_report_count;

		//AoS - xmachine_message_location_report Un-Coalesced scattered memory write
		messages->_position[output_index] = output_index;
		messages->id[output_index] = messages_swap->id[index];
		messages->x[output_index] = messages_swap->x[index];
		messages->y[output_index] = messages_swap->y[index];
...
/** reset_location_report_swaps
 * Reset non partitioned or spatially partitioned location_report message swaps (for scattering optional messages)
 * @param message_swap message list to reset _position and _scan_input values back to 0
 */
__global__ void reset_location_report_swaps(xmachine_message_location_report_list* messages_swap){
...
/** message_location_report_grid_position
 * Calculates the grid cell position given an glm::vec3 vector
 * @param position glm::vec3 vector representing a position
 */
__device__ glm::ivec3 message_location_report_grid_position(glm::vec3 position)
{
    glm::ivec3 gridPos;
    gridPos.x = floor((position.x - d_message_location_report_min_bounds.x) * (float)d_message_location_report_partitionDim.x / (d_message_location_report_max_bounds.x - d_message_location_report_min_bounds.x));
    gridPos.y = floor((position.y - d_message_location_report_min_bounds.y) * (float)d_message_location_report_partitionDim.y / (d_message_location_report_max_bounds.y - d_message_location_report_min_bounds.y));
    gridPos.z = floor((position.z - d_message_location_report_min_bounds.z) * (float)d_message_location_report_partitionDim.z / (d_message_location_report_max_bounds.z - d_message_location_report_min_bounds.z));
...
/** message_location_report_hash
 * Given the grid position in partition space this function calculates a hash value
 * @param gridPos The position in partition space
 */
__device__ unsigned int message_location_report_hash(glm::ivec3 gridPos)
{
	//cheap bounding without mod (within range +- partition dimension)
	gridPos.x = (gridPos.x<0)? d_message_location_report_partitionDim.x-1: gridPos.x; 
	gridPos.x = (gridPos.x>=d_message_location_report_partitionDim.x)? 0 : gridPos.x; 
	gridPos.y = (gridPos.y<0)? d_message_location_report_partitionDim.y-1 : gridPos.y; 
	gridPos.y = (gridPos.y>=d_message_location_report_partitionDim.y)? 0 : gridPos.y; 
	gridPos.z = (gridPos.z<0)? d_message_location_report_partitionDim.z-1: gridPos.z; 
	gridPos.z = (gridPos.z>=d_message_location_report_partitionDim.z)? 0 : gridPos.z; 

	//unique id
	return ((gridPos.z * d_message_location_report_partitionDim.y) * d_message_location_report_partitionDim.x) + (gridPos.y * d_message_location_report_partitionDim.x) + gridPos.x;
}

#ifdef FAST_ATOMIC_SORTING
	/** hist_location_report_messages
		 * Kernal function for performing a histogram (count) on each partition bin and saving the hash and index of a message within that bin
		 * @param local_bin_index output index of the message within the calculated bin
		 * @param unsorted_index output bin index (hash) value
		 * @param messages the message list used to generate the hash value outputs
		 * @param agent_count the current number of agents outputting messages
		 */
	__global__ void hist_location_report_messages(uint* local_bin_index, uint* unsorted_index, int* global_bin_count, xmachine_message_location_report_list* messages, int agent_count)
	{
		unsigned int index = (blockIdx.x * blockDim.x) + threadIdx.x;
		...
		if (index >= agent_count)
			return;
        glm::vec3 position = glm::vec3(messages->x[index], messages->y[index], messages->z[index]);
		glm::ivec3 grid_position = message_location_report_grid_position(position);
		unsigned int hash = message_location_report_hash(grid_position);
		unsigned int bin_idx = atomicInc((unsigned int*) &global_bin_count[hash], 0xFFFFFFFF);
		local_bin_index[index] = bin_idx;
		unsorted_index[index] = hash;
	}
	
	/** reorder_location_report_messages
	 * Reorders the messages accoring to the partition boundary matrix start indices of each bin
	 * @param local_bin_index index of the message within the desired bin
	 * @param unsorted_index bin index (hash) value
	 * @param pbm_start_index the start indices of the partition boundary matrix
	 ...
	 * @param ordered_messages buffer used to scatter messages into the correct order
	  @param agent_count the current number of agents outputting messages
	 */
	 __global__ void reorder_location_report_messages(uint* local_bin_index, uint* unsorted_index, int* pbm_start_index, xmachine_message_location_report_list* unordered_messages, xmachine_message_location_report_list* ordered_messages, int agent_count)
	{
		int index = (blockIdx.x *blockDim.x) + threadIdx.x;
	...
	/** hash_location_report_messages
	 * Kernal function for calculating a hash value for each messahe depending on its position
	 * @param keys output for the hash key
	 * @param values output for the index value
	 * @param messages the message list used to generate the hash value outputs
	 */
	__global__ void hash_location_report_messages(uint* keys, uint* values, xmachine_message_location_report_list* messages)
	{
		unsigned int index = (blockIdx.x * blockDim.x) + threadIdx.x;
        glm::vec3 position = glm::vec3(messages->x[index], messages->y[index], messages->z[index]);
		glm::ivec3 grid_position = message_location_report_grid_position(position);
	 ...
	 * @param ordered_messages buffer used to scatter messages into the correct order
	 */
	__global__ void reorder_location_report_messages(uint* keys, uint* values, xmachine_message_location_report_PBM* matrix, xmachine_message_location_report_list* unordered_messages, xmachine_message_location_report_list* ordered_messages)
	{
		extern __shared__ int sm_data [];
		...
		//set partition boundaries
		if (index < d_message_location_report_count)
		{
			//if first thread then set first partition cell start
			if (index == 0)
			{
			...
			//if last thread then set final partition cell end
			if (index == d_message_location_report_count-1)
			{
				matrix->end_or_count[key] = index+1;
			}
		}
...
/** load_next_location_report_message
 * Used to load the next message data to shared memory
 * Idea is check the current cell index to see if we can simply get a message from the current cell
 * If we are at the end of the current cell then loop till we find the next cell with messages (this way we ignore cells with no messages)
 * @param messages the message list
 ...
 * @return true if a message has been loaded into sm false otherwise
 */
__device__ bool load_next_location_report_message(xmachine_message_location_report_list* messages, xmachine_message_location_report_PBM* partition_matrix, glm::ivec3 relative_cell, int cell_index_max, glm::ivec3 agent_grid_cell, int cell_index)
{
	extern __shared__ int sm_data [];
	char* message_share = (char*)&sm_data[0];
	...
	while(move_cell)
	{
        ...
        if (next_cell3D(&relative_cell))
		{
			//calculate the next cells grid position and hash
			glm::ivec3 next_cell_position = agent_grid_cell + relative_cell;
			int next_cell_hash = message_location_report_hash(next_cell_position);
			//use the hash to calculate the start index
			int cell_index_min = tex1Dfetch(tex_xmachine_message_location_report_pbm_start, next_cell_hash + d_tex_xmachine_message_location_report_pbm_start_offset);
			cell_index_max = tex1Dfetch(tex_xmachine_message_location_report_pbm_end_or_count, next_cell_hash + d_tex_xmachine_message_location_report_pbm_end_or_count_offset);
			//check for messages in the cell (cell index max is the count for atomic sorting)
#ifdef FAST_ATOMIC_SORTING
			if (cell_index_max > 0)
			{
	...
	//get message data using texture fetch
	xmachine_message_location_report temp_message;
	temp_message._relative_cell = relative_cell;
	temp_message._cell_index_max = cell_index_max;
	temp_message._cell_index = cell_index;
	temp_message._agent_grid_cell = agent_grid_cell;
	...
	//Using texture cache
  temp_message.id = tex1Dfetch(tex_xmachine_message_location_report_id, cell_index + d_tex_xmachine_message_location_report_id_offset); temp_message.x = tex1Dfetch(tex_xmachine_message_location_report_x, cell_index + d_tex_xmachine_message_location_report_x_offset); temp_message.y = tex1Dfetch(tex_xmachine_message_location_report_y, cell_index + d_tex_xmachine_message_location_report_y_offset); temp_message.z = tex1Dfetch(tex_xmachine_message_location_report_z, cell_index + d_tex_xmachine_message_location_report_z_offset); 

	//load it into shared memory (no sync as no sharing between threads)
	int message_index = SHARE_INDEX(threadIdx.y*blockDim.x+threadIdx.x, sizeof(xmachine_message_location_report));
	xmachine_message_location_report* sm_message = ((xmachine_message_location_report*)&message_share[message_index]);
	sm_message[0] = temp_message;
...
/*
 * get first spatial partitioned location_report message (first batch load into shared memory)
 */
__device__ xmachine_message_location_report* get_first_location_report_message(xmachine_message_location_report_list* messages, xmachine_message_location_report_PBM* partition_matrix, float x, float y, float z){

	extern __shared__ int sm_data [];
	char* message_share = (char*)&sm_data[0];

	// If there are no messages, do not load any messages
	if(d_message_location_report_count == 0){
		return nullptr;
	}
	...
	int cell_index = 0;
	glm::vec3 position = glm::vec3(x, y, z);
	glm::ivec3 agent_grid_cell = message_location_report_grid_position(position);
	
	if (load_next_location_report_message(messages, partition_matrix, relative_cell, cell_index_max, agent_grid_cell, cell_index))
	{
		int message_index = SHARE_INDEX(threadIdx.y*blockDim.x+threadIdx.x, sizeof(xmachine_message_location_report));
		return ((xmachine_message_location_report*)&message_share[message_index]);
	}
	else
	{
		return nullptr;
...
/*
 * get next spatial partitioned location_report message (either from SM or next batch load)
 */
__device__ xmachine_message_location_report* get_next_location_report_message(xmachine_message_location_report* message, xmachine_message_location_report_list* messages, xmachine_message_location_report_PBM* partition_matrix){
	...
	if (load_next_location_report_message(messages, partition_matrix, message->_relative_cell, message->_cell_index_max, message->_agent_grid_cell, message->_cell_index))
	{
		//get conflict free address of 
		int message_index = SHARE_INDEX(threadIdx.y*blockDim.x+threadIdx.x, sizeof(xmachine_message_location_report));
		return ((xmachine_message_location_report*)&message_share[message_index]);
 ...
 *
 */
__global__ void GPUFLAME_ProliferationCompletion(xmachine_memory_Fibroblast_list* agents){
	...
	//SoA to AoS - xmachine_memory_ProliferationCompletion Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_Fibroblast agent;
	...
	//FLAME function call
	int dead = !ProliferationCompletion(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_ProliferationCompletion Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
 ...
 *
 */
__global__ void GPUFLAME_BystanderEffect(xmachine_memory_Fibroblast_list* agents, xmachine_message_location_report_list* location_report_messages, xmachine_message_location_report_PBM* partition_matrix){
	...
	//FLAME function call
	int dead = !BystanderEffect(&agent, location_report_messages, partition_matrix);
 ...
 *
 */
__global__ void GPUFLAME_ExcessiveDamage(xmachine_memory_Fibroblast_list* agents, xmachine_message_fibroblast_damage_report_list* fibroblast_damage_report_messages, xmachine_message_fibroblast_damage_report_PBM* partition_matrix){
	...
	//FLAME function call
	int dead = !ExcessiveDamage(&agent, fibroblast_damage_report_messages, partition_matrix);
 ...
 *
 */
__global__ void GPUFLAME_ReplicativeSenescence(xmachine_memory_Fibroblast_list* agents, xmachine_message_doublings_list* doublings_messages){
	...
	//FLAME function call
	int dead = !ReplicativeSenescence(&agent, doublings_messages);
