...
__constant__ int d_xmachine_memory_A_count;

__constant__ int d_xmachine_memory_B_count;

/* Agent state count constants */

__constant__ int d_xmachine_memory_A_moving_A_count;

__constant__ int d_xmachine_memory_A_change_direction_A_count;

__constant__ int d_xmachine_memory_A_get_going_again_A_count;

__constant__ int d_xmachine_memory_B_moving_B_count;

__constant__ int d_xmachine_memory_B_change_direction_B_count;

__constant__ int d_xmachine_memory_B_get_going_again_B_count;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created A agent functions */

/** reset_A_scan_input
 * A agent reset scan input function
 * @param agents The xmachine_memory_A_list agent list
 */
__global__ void reset_A_scan_input(xmachine_memory_A_list* agents){
...
/** scatter_A_Agents
 * A scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_A_list agent list destination
 * @param agents_src xmachine_memory_A_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void scatter_A_Agents(xmachine_memory_A_list* agents_dst, xmachine_memory_A_list* agents_src, int dst_agent_count, int number_to_scatter){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
 ...
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void append_A_Agents(xmachine_memory_A_list* agents_dst, xmachine_memory_A_list* agents_src, int dst_agent_count, int number_to_append){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** add_A_agent
 * Continuous A agent add agent function writes agent data to agent swap
 * @param agents xmachine_memory_A_list to add agents to 
...
__device__ void add_A_agent(xmachine_memory_A_list* agents, int id, float x, float y, float z, float fx, float fy, float fz, int colour){
...
__device__ void add_A_agent(xmachine_memory_A_list* agents, int id, float x, float y, float z, float fx, float fy, float fz, int colour){
    add_A_agent<DISCRETE_2D>(agents, id, x, y, z, fx, fy, fz, colour);
}

/** reorder_A_agents
 * Continuous A agent areorder function used after key value pairs have been sorted
 ...
 */
__global__ void reorder_A_agents(unsigned int* values, xmachine_memory_A_list* unordered_agents, xmachine_memory_A_list* ordered_agents)
{
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created B agent functions */

/** reset_B_scan_input
 * B agent reset scan input function
 * @param agents The xmachine_memory_B_list agent list
 */
__global__ void reset_B_scan_input(xmachine_memory_B_list* agents){
...
/** scatter_B_Agents
 * B scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_B_list agent list destination
 * @param agents_src xmachine_memory_B_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void scatter_B_Agents(xmachine_memory_B_list* agents_dst, xmachine_memory_B_list* agents_src, int dst_agent_count, int number_to_scatter){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
 ...
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void append_B_Agents(xmachine_memory_B_list* agents_dst, xmachine_memory_B_list* agents_src, int dst_agent_count, int number_to_append){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** add_B_agent
 * Continuous B agent add agent function writes agent data to agent swap
 * @param agents xmachine_memory_B_list to add agents to 
 * @param id agent variable of type int
 * @param x agent variable of type float
 * @param y agent variable of type float
 * @param z agent variable of type float
 ...
 */
template <int AGENT_TYPE>
__device__ void add_B_agent(xmachine_memory_B_list* agents, int id, float x, float y, float z, float fx, float fy, float fz, int colour){
...
//non templated version assumes DISCRETE_2D but works also for CONTINUOUS
__device__ void add_B_agent(xmachine_memory_B_list* agents, int id, float x, float y, float z, float fx, float fy, float fz, int colour){
    add_B_agent<DISCRETE_2D>(agents, id, x, y, z, fx, fy, fz, colour);
}

/** reorder_B_agents
 * Continuous B agent areorder function used after key value pairs have been sorted
 * @param values sorted index values
 * @param unordered_agents list of unordered agents
 * @ param ordered_agents list used to output ordered agents
 */
__global__ void reorder_B_agents(unsigned int* values, xmachine_memory_B_list* unordered_agents, xmachine_memory_B_list* ordered_agents)
{
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
 ...
 *
 */
__global__ void GPUFLAME_move_A(xmachine_memory_A_list* agents){
	
	//continuous agent: index is agent position in 1D agent list
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
  
    //For agents not using non partitioned message input check the agent bounds
    if (index >= d_xmachine_memory_A_count)
        return;
    

	//SoA to AoS - xmachine_memory_move_A Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_A agent;
	...
	//FLAME function call
	int dead = !move_A(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_move_A Coalesced memory write (ignore arrays)
 ...
 *
 */
__global__ void GPUFLAME_reverse_direction_A(xmachine_memory_A_list* agents){
	...
	//SoA to AoS - xmachine_memory_reverse_direction_A Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_A agent;
	...
	//FLAME function call
	int dead = !reverse_direction_A(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_reverse_direction_A Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
 ...
 *
 */
__global__ void GPUFLAME_resume_movement_A(xmachine_memory_A_list* agents){
	...
	//SoA to AoS - xmachine_memory_resume_movement_A Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_A agent;
	...
	//FLAME function call
	int dead = !resume_movement_A(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_resume_movement_A Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
 ...
 *
 */
__global__ void GPUFLAME_move_B(xmachine_memory_B_list* agents){
	
	//continuous agent: index is agent position in 1D agent list
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
  
    //For agents not using non partitioned message input check the agent bounds
    if (index >= d_xmachine_memory_B_count)
        return;
    

	//SoA to AoS - xmachine_memory_move_B Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_B agent;
	...
	//FLAME function call
	int dead = !move_B(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_move_B Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
 ...
 *
 */
__global__ void GPUFLAME_reverse_direction_B(xmachine_memory_B_list* agents){
	...
	//SoA to AoS - xmachine_memory_reverse_direction_B Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_B agent;
	...
	//FLAME function call
	int dead = !reverse_direction_B(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_reverse_direction_B Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
 ...
 *
 */
__global__ void GPUFLAME_resume_movement_B(xmachine_memory_B_list* agents){
	...
	//SoA to AoS - xmachine_memory_resume_movement_B Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_B agent;
	...
	//FLAME function call
	int dead = !resume_movement_B(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_resume_movement_B Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
